...
new eventPromised_1.EventPromised((resolve, reject, emit) => {
    ...
    (async () => {
        ...
        const { name, theme, path, typescript, noGit } = steps_1.normalizeOptions(defaultOptions, options);
        ...
        try {
            ...
            if (!noGit && utils_1.hasGit()) {
                const gitignoreCleanup = await steps_1.createGitignore(path);
                if (utils_1.isInGitRepository()) {
                    emit("message", "Already in a git repository. Skipping initialization of a new git repo.", step);
                }
                else {
                    step = steps_1.initializeGit(path);
